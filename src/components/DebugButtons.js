  ...
  function requestAirdrop() {
    callAsync(
      wallet.connection.requestAirdrop(wallet.publicKey, LAMPORTS_PER_SOL),
      {
        ...
        successMessage:
          'Success! Please wait up to 30 seconds for the KUNCI tokens to appear in your wallet.',
      },
    );
  }
  ...
  return (
    <div style={{ display: 'flex', marginLeft: spacing }}>
      <Tooltip
        title={
          requestAirdropDisabled
            ? 'Receive some devnet KUNCI for free. Only enabled on the devnet'
            : 'Receive some devnet KUNCI for free'
        }
      >
        <span>
          <Button
      ...
      <Tooltip
        title={
          noSol
            ? 'Generate and receive balances in a new test token. Requires KUNCI balance'
